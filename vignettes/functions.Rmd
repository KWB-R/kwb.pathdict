...
`get_subdir_frequencies()` is called. This function either gets a vector of path
strings or a list of character vectors representing all parts of the paths as
input. For each possible path length it determines all different sub-paths and
orders them by their "importance", i.e. the product of frequency and path length
in number of characters. By default, only the most important path per depth 
level is returned:
